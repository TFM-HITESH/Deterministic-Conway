...
def main():
    ...
    pl1.plot(range(0,CYCLES), X_VALS, marker='o', linestyle='', markersize=8, color='darksalmon', label='Scatter Plot' )
    # thetaX = np.polyfit(range(0,CYCLES), X_VALS, 1)
    yLineX = [slopeMean] * len(X_VALS)
    # print("y = ", round(thetaX[0],4), "* x +", round(thetaX[1],4))
    pl1.set_title("Values of Slope(m) for regression obtained over the results from EEEGOL")
    pl1.set_ylabel("Value of Slope for EEEGOL Population")
    pl1.plot(range(0,CYCLES), yLineX, 'maroon')
    ...
    pl2.set_title("Values of Intercept(c) for regression obtained over the results from EEEGOL")
    pl2.set_ylabel("Value of Intercept for EEEGOL Population")
